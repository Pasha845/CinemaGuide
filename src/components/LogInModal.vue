<template>
  <div class="modal" v-if="isLogInModalOpen">
    <div class="modal-container">
      <button class="modal-exit" @click.prevent="$emit('close'), logInErrorEmail = false, logInErrorPass = false">
      ...
      <form class="modal-form" @submit.prevent="logInModal">
        <label class="modal-label flex" :class="{'modal-label-error' : logInErrorEmail}">
          ...
          <input class="modal-input" v-model="logInEmail" type="email" placeholder="Email">
        </label>
        <label class="modal-label modal-label-last flex" :class="{'modal-label-error' : logInErrorPass}">
      ...
      <button class="modal-link" @click.prevent="$emit('close'), isError = false, isSignInModalOpen = true">Sign up</button>
    </div>
  </div>

  <div class="modal" v-if="isSignInModalOpen">
    <div class="modal-container modal-regist">
      <button class="modal-exit" @click="isSignInModalOpen = false">
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M8.5859 10L0.792969 2.20706L2.20718 0.792847L10.0001 8.5857L17.793 0.792847L19.2072 2.20706L11.4143 10L19.2072 17.7928L17.793 19.2071L10.0001 11.4142L2.20718 19.2071L0.792969 17.7928L8.5859 10Z" fill="black"/>
        </svg>
      </button>
      ...
      <form class="modal-form" @submit.prevent="signInModal">
        <label class="modal-label flex">
          ...
          <input class="modal-input" v-model="signInEmail" type="email" placeholder="Email" required>
        </label>
        <label class="modal-label flex">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M4 22C4 17.5817 7.58172 14 12 14C16.4183 14 20 17.5817 20 22H18C18 18.6863 15.3137 16 12 16C8.68629 16 6 18.6863 6 22H4ZM12 13C8.685 13 6 10.315 6 7C6 3.685 8.685 1 12 1C15.315 1 18 3.685 18 7C18 10.315 15.315 13 12 13ZM12 11C14.21 11 16 9.21 16 7C16 4.79 14.21 3 12 3C9.79 3 8 4.79 8 7C8 9.21 9.79 11 12 11Z" fill="currentColor"/>
          </svg>
          <input class="modal-input" v-model="signInName" type="text" placeholder="Name" required>
        </label>
        <label class="modal-label flex">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M4 22C4 17.5817 7.58172 14 12 14C16.4183 14 20 17.5817 20 22H18C18 18.6863 15.3137 16 12 16C8.68629 16 6 18.6863 6 22H4ZM12 13C8.685 13 6 10.315 6 7C6 3.685 8.685 1 12 1C15.315 1 18 3.685 18 7C18 10.315 15.315 13 12 13ZM12 11C14.21 11 16 9.21 16 7C16 4.79 14.21 3 12 3C9.79 3 8 4.79 8 7C8 9.21 9.79 11 12 11Z" fill="currentColor"/>
          </svg>
          <input class="modal-input" v-model="signInSurname" type="text" placeholder="Surname" required>
        </label>
        <label class="modal-label flex">
          <svg width="22" height="12" viewBox="0 0 22 12" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M11.917 7C11.441 9.8377 8.973 12 6 12C2.68629 12 0 9.3137 0 6C0 2.68629 2.68629 0 6 0C8.973 0 11.441 2.16229 11.917 5H22V7H20V11H18V7H16V11H14V7H11.917ZM6 10C8.20914 10 10 8.2091 10 6C10 3.79086 8.20914 2 6 2C3.79086 2 2 3.79086 2 6C2 8.2091 3.79086 10 6 10Z" fill="currentColor"/>
          </svg>
          <input class="modal-input" v-model="signInPass" type="password" placeholder="Password" required>
        </label>
        <label class="modal-label modal-label-last flex">
          <svg width="22" height="12" viewBox="0 0 22 12" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M11.917 7C11.441 9.8377 8.973 12 6 12C2.68629 12 0 9.3137 0 6C0 2.68629 2.68629 0 6 0C8.973 0 11.441 2.16229 11.917 5H22V7H20V11H18V7H16V11H14V7H11.917ZM6 10C8.20914 10 10 8.2091 10 6C10 3.79086 8.20914 2 6 2C3.79086 2 2 3.79086 2 6C2 8.2091 3.79086 10 6 10Z" fill="currentColor"/>
          </svg>
          <input class="modal-input" v-model="signInTwoPass" type="password" placeholder="Confirm password" required>
        </label>
        <button class="btn modal-btn mb-24" type="submit">Create an account</button>
      </form>
      <button class="modal-link" @click.prevent="$emit('open'), isSignInModalOpen = false">I have a password</button>
    </div>
  </div>
...
<script setup lang="ts">
  ...
  const logInErrorEmail = ref(false);
  const logInErrorPass = ref(false);
  ...
  watch(signInEmail, () => {
    signInUp.value.email = signInEmail.value;
  });

  watch(signInName, () => {
    signInUp.value.name = signInName.value;
  });

  watch(signInSurname, () => {
    signInUp.value.surname = signInSurname.value;
  });

  watch(signInPass, () => {
    signInUp.value.pass = signInPass.value;
  });

  watch(signInTwoPass, () => {});

  watch(logInEmail, () => {
    logIn.value.email = logInEmail.value;
  });

  watch(logInPass, () => {
    logIn.value.pass = logInPass.value;
  });
