<template>
  <section class="hero">
    <div class="hero__container container">
      <img v-if="!cardIsLoading" @load="handleLoad" :src="random.backdropUrl" alt="Film image" width="900" height="680">
      <div v-else class="loader"></div>
      <div class="hero__item">
        <div class="hero__wrapper flex">
          <p class="hero__rating flex" :class="[{'green' : greenRating, 'gray' : grayRating, 'red' : random.tmdbRating < 4}, 'yellow']">
          ...
          <p class="hero__length">
            {{ oneHour ? ' 1 h ' + (random.runtime - 60) + ' min' : twoHours ?' 2 h ' + (random.runtime - 120) + ' min' : random.runtime > 180 ? ' 3 h ' + (random.runtime - 180) + ' min' : random.runtime < 60 + ' min' ? random.runtime == null : '' }}
          </p>
        </div>
        ...
        <div class="hero__cube flex">
          ...
          <button class="hero__sublink" @click="loadRandomFilms">
...
<script setup lang="ts">
  ...
  const greenRating = computed(() => {
    return random.tmdbRating >= 7 && random.tmdbRating < 8
  });

  const grayRating = computed(() => {
    return random.tmdbRating <= 7 && random.tmdbRating >= 4
  });

  const oneHour = computed(() => {
    return random.runtime > 60 && random.runtime < 120
  });

  const twoHours = computed(() => {
    return random.runtime > 120 && random.runtime < 180
  });
